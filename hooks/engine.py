...
class HookEngine:
    ...
    def _notify_listeners(self, event: str, ctx: HookContext) -> None:
        for listener in list(self._listeners):
            try:
                handler = getattr(listener, "handle_hook_event", None)
                result = handler(event, ctx) if callable(handler) else listener(event, ctx)
                if inspect.isawaitable(result):
                    asyncio.ensure_future(result)
            except Exception as e:
                log.debug("Hook listener failed for event '%s': %s", event, e)
    ...
    async def run_hooks(self, event: str, ctx: HookContext) -> None:
        self._notify_listeners(event, ctx)
    ...
    async def run_pre_tool_hooks(
        ...
        self._notify_listeners("pre_tool_use", ctx)
