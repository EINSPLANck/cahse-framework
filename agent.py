...
class Agent:
    def __init__(
        ...
        evolution_collector: Any | None = None,
    ) -> None:
        self.client = client
        ...
        self.evolution_collector = evolution_collector
        self._loop_count = 0
    ...
    def _build_hook_context(self, event: str, **kwargs: Any) -> HookContext:
        return HookContext(
            ...
            elapsed=float(kwargs.get("elapsed", 0.0) or 0.0),
            session_id=str(kwargs.get("session_id", self.session_id)),
            task_description=str(kwargs.get("task_description", "")),
            active_skills=dict(kwargs.get("active_skills", self.active_skills) or {}),
    ...
    async def _run_pre_tool_hooks_for_call(
        ...
        return await self.hook_engine.run_pre_tool_hooks(hook_ctx)

    async def _run_post_tool_hooks_for_result(
        self,
        tc: ToolCallComplete,
        result: ToolResult,
        elapsed: float,
    ...
    def _task_description_from_conversation(
        self,
    ...
    def _start_evolution_task(
        self,
        conversation: ConversationManager,
        *,
        execution_context: str = "",
    ) -> None:
        collector = self.evolution_collector
        if collector is None:
            return
        set_session_id = getattr(collector, "set_session_id", None)
        if callable(set_session_id):
            set_session_id(self.session_id)
        start_task = getattr(collector, "start_task", None)
        if callable(start_task):
            task_description = self._task_description_from_conversation(conversation)
            try:
                start_task(task_description, execution_context=execution_context)
            except TypeError:
                start_task(task_description)

    def _finish_evolution_task(
        self,
        success: bool | None,
        final_message: str = "",
    ) -> None:
        ...
        if collector is None:
            return
        finish_task = getattr(collector, "finish_task", None)
        if callable(finish_task):
            finish_task(success, final_message)

    def _record_evolution_tool_call(self, tc: ToolCallComplete) -> None:
        collector = self.evolution_collector
        if collector is None:
            return
        record = getattr(collector, "record_tool_call", None)
        if callable(record):
            record(tc.tool_id, tc.tool_name, tc.arguments)

    def _record_evolution_tool_result(
        self,
        tc: ToolCallComplete,
        result: ToolResult,
        elapsed: float,
        ...
        if collector is None:
            return
        record = getattr(collector, "record_tool_result", None)
        if callable(record):
            record(tc.tool_id, tc.tool_name, result.output, result.is_error, elapsed)
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        execution_context = TaskAnalyzer().build_execution_context(
            task_description,
            memory_context=memory_content,
            active_skills=self.active_skills,
            repository_context=env_context,
        )
        self._start_evolution_task(
            conversation, execution_context=execution_context
        )
        ...
        if self.hook_engine:
            ctx = self._build_hook_context(
                ...
                metadata={
                    ...
                    "memory_context_available": bool(memory_content),
                },
            )
        ...
        while True:
            ...
            if self.max_iterations > 0 and iteration > self.max_iterations:
                self._finish_evolution_task(False, "maximum iterations reached")
                yield ErrorEvent(
            ...
            if not response.tool_calls:
                ...
                if self.hook_engine:
                    ...
                    ctx = self._build_hook_context("session_end", message=response.text, metadata={"task_success": True}, active_skills=self.active_skills)
                ...
                self._finish_evolution_task(True, response.text)
            ...
            if consecutive_unknown >= 3:
                self._finish_evolution_task(False, "too many consecutive unknown tool calls")
                yield ErrorEvent(
    ...
    async def _execute_single_tool_direct(
        ...
        self._record_evolution_tool_call(tc)
        tool = self.registry.get(tc.tool_name)
        start = time.monotonic()

        rejection = await self._run_pre_tool_hooks_for_call(tc)
        if rejection is not None:
            result = ToolResult(output=f"Hook rejected: {rejection.reason}", is_error=True)
            elapsed = time.monotonic() - start
            await self._run_post_tool_hooks_for_result(tc, result, elapsed)
            self._record_evolution_tool_result(tc, result, elapsed)
    ...
    async def run_to_completion(
        ...
        self._start_evolution_task(conversation)
